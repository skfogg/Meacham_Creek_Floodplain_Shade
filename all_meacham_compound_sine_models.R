...
for (i in 1:3){
#f567432c350fe53da339891f1b40b3aefaadb03a
### --------------------------- DAILY MEAN MODEL --------------------------- ###
### ------------------------------------------------------------------------ ###
# start val for m:
...
annual_sig <- coef(dailymeanmodel)[1] + coef(dailymeanmodel)[2]*sin(2*pi*(1/31536000)*t10 + coef(dailymeanmodel)[3])
write.table(data.frame(s = seq(0, by = 3600, length.out = 87601), e = seq(3600, by = 3600, length.out = 87601), annual_sig), "meacham_water_annual.txt", row.names = F, col.names = F)
...
#<<<<<<< HEAD
plot(rawdata[[i]]$Temp - rawdata[[i]]$DailyMean, type = "l", xlim = c(2000, 2000+(24*2)))
lines(predict(dailyfreqmodel), col = "goldenrod")
...
#<<<<<<< HEAD
meachammodels[[i]] <- means + amps*sin(2*pi*(1/86400)*t10 - 2.25) #coef(dailyfreqmodel)[2]
#=======
...
plot(meachammodels[[3]] ~ t10, type = "l",
     ylim = c(0,25), xlim = c(1.5*10^7, 1.5*10^7 + (86400*7))) # c(1.0*10^7, 5.0*10^7)
#=======
plot(meachammodels[[1]] ~ t2, type = "l",
     ylim = c(0,35))
lines(rawdata[[1]]$Temp ~ rawdata[[1]]$NumTimeOfYr,
...
write.csv(meachamwaterdf, "meachamwaterdf.csv")
#=======
meachamshadysoil <- meachammodels[[1]]
meachamsunnysoil <- meachammodels[[2]]
meachamwatertemp <- meachammodels[[3]]
